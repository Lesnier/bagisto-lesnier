...
<div class="flex flex-wrap gap-4 px-4 pt-6 pb-4 shadow-sm lg:hidden">
    <div class="flex items-center justify-between w-full">
        ...
        <!-- Become a Vendor Button (Mobile/Tablet) -->
        <div class="hidden md:flex ml-2 mr-auto items-center">
            @auth('customer')
                @if(! auth()->guard('customer')->user()->is_vendor())
                    <a href="{{ route('marketplace.vendor.apply.create') }}" class="px-3 py-1.5 text-xs font-semibold text-white bg-navyBlue rounded-lg hover:bg-navyBlue-800 transition-all">
                        Convertirme en Vendor
                    </a>
                @endif
            @endauth
            @guest('customer')
                <a href="{{ route('shop.customer.session.create') }}" class="px-3 py-1.5 text-xs font-semibold text-white bg-navyBlue rounded-lg hover:bg-navyBlue-800 transition-all">
                    Convertirme en Vendor
                </a>
            @endguest
        </div>
                        ...
                        @auth('customer')
                            <x-slot:content class="!p-0">
                                ...
                                <div class="mt-2.5 grid gap-1 pb-2.5">
                                    ...
                                    @if(auth()->guard('customer')->user()->is_vendor())
                                        <a
                                            class="px-5 py-2 text-base cursor-pointer font-semibold text-indigo-700"
                                            href="{{ route('marketplace.vendor.profile', ['slug' => auth()->guard('customer')->user()->vendor->shop_slug]) }}"
                                        >
                                            Mi Tienda
                                        </a>
                                        <a
                                            class="px-5 py-2 text-base cursor-pointer font-semibold text-blue-700"
                                    ...
                                    @else
                                        <a
                                            class="px-5 py-2 text-base cursor-pointer font-medium"
                                            href="{{ route('marketplace.vendor.apply.create') }}"
                                        >
                                            Convertirme en Vendor
                                        </a>
                                    @endif
